...
public class App {
  ...
  public void countWords() {
    ...
    Map<String, Long> du =
        elements.stream().collect(Collectors.groupingBy(String::valueOf, Collectors.counting()));
    // du.entrySet().forEach(System.out::println);
    du.entrySet().stream().filter(c -> c.getValue() >= 2).forEach(System.out::println);
  }

  public void palindrome() {
    List<String> words = Stream.of("aba", "cdc", "dea").toList();
    List<String> palinDrome = words.stream().filter(this::palindromeUsingStringBuilder).toList();
